...
public class LeetcodeArgsParser implements IParser {
    public LeetcodeArgsParser() {
    }
...
//        }
        ...
        for (String s : output) {
            ...
            if(output[i].contains("[") && output[i].contains("]")){
                output[i] = cutFirstAndLast(new StringBuilder(output[i])).toString();
            }
